...
## Assembly and file operations (the set of buttons below the memory table) {#assembly-and-file-operations-the-set-of-buttons-below-the-memory-table}

The memory toolbar provides these actions:

-   *Reset*: clear all memory bytes to `00`
-   *Save to File*: save the full 256-byte memory image as a binary file
-   *Load from File*: load a 256-byte binary memory image, replacing the current memory
-   *Assemble and load*: read a UTF-8 assembly source file, assemble it, and load the resulting 256-byte image into memory
-   *Assemble to file*: assemble a UTF-8 assembly source file and save the resulting 256-byte image without changing memory
